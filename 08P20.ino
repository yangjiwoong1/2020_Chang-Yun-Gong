...
float dist_min, dist_max, dist_raw,dist_raw_past; // unit: mm
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw == 0 || dist_raw == dist_min||dist_raw == dist_max) {
    analogWrite(PIN_LED, 255);
  }
  else {
    float temp=0+abs(200-dist_raw)*gap ;
    analogWrite(PIN_LED, temp);
  }
  
  last_sampling_time += INTERVAL;
  dist_raw_past = dist_raw;
}
